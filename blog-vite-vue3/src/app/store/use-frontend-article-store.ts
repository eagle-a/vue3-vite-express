...
import { piniaInit } from './index'
import type { ApiConfig, ApiType, Article, FArticleStore, ResDataList, ResDataLists } from '@/types'
...
const usePiniaStore = defineStore('frontendArticleStore', () => {
    ...
    const getArticleItem = async (config: ApiConfig, $api: ApiType = api) => {
        ...
        if (code === 200 && data) {
            ...
            // 请求失败也设置 isLoad 为 true，表示加载完成（但数据为 null）
            state.item = {
                data: null,
                ...config,
                isLoad: true,
