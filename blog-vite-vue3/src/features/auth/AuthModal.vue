        ...
        <Transition name="modal">
            <div v-if="modelValue" class="modal-overlay" @click.self="closeModal">
                <GlassPanel class="modal-container" :blur="20" :opacity="0.95">
                    <button class="modal-close" aria-label="关闭" @click="closeModal">
...
import GlassPanel from '@/components/base/GlassPanel.vue'
...
async function handleSubmit() {
    ...
    loading.value = true
    try {
        const endpoint = isLogin.value ? 'frontend/user/login' : 'frontend/user/register'
        const payload = isLogin.value ? { username: form.email, password: form.password } : { username: form.username, email: form.email, password: form.password }

        const { code, message } = await api.post<UserCookies>(endpoint, payload)
