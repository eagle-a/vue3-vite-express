...
(async () => {
    ...
    const jsonList = files.filter(str => str.endsWith('.json'))
    const flmList = files.filter(str => str.toLowerCase().endsWith('.flm'))
    ...
    for (const item of files) {
        ...
        const [initAddr] = get_symbol_info(elfInfo, 'Function', 'Init')
        const [unInitAddr] = get_symbol_info(elfInfo, 'Function', 'UnInit')
        const [eraseChipAddr] = get_symbol_info(elfInfo, 'Function', 'EraseChip')
        const [eraseSectorAddr] = get_symbol_info(elfInfo, 'Function', 'EraseSector')
        const [programPageAddr] = get_symbol_info(elfInfo, 'Function', 'ProgramPage')
        const [descAddr, descSize] = get_symbol_info(elfInfo, 'Object', 'FlashDevice')

        const [dataSectionAddr] = get_symbol_info(elfInfo, 'Section', '.data')
        const [bssSectionAddr] = get_symbol_info(elfInfo, 'Section', '.bss')
        ...
        const rawDataDesc = new Uint8Array(descSize)
        const ret = await fd.read(rawDataDesc, 0, descSize, descAddr)
        ...
        const devDesc = flashDescript.fromBuffer(rawDataDesc)
